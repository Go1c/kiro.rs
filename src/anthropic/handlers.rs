...
use super::cache::{self, CacheResult};
...
pub async fn post_messages(
    ...
    let cache_result = compute_cache_result(&headers, &payload);
...
fn compute_cache_result(headers: &HeaderMap, payload: &MessagesRequest) -> CacheResult {
...
async fn handle_non_stream_request(
    ...
    // Anthropic usage 中 input_tokens 表示未从 prompt cache 读取/创建的部分。
    let final_input_tokens = cache_result.input_tokens_after_cache(context_input_tokens);
