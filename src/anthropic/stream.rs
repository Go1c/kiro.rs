...
impl StreamContext {
    ...
    /// 创建带 prompt cache usage 的 StreamContext
    ...
    pub fn generate_final_events(&mut self) -> Vec<SseEvent> {
        ...
        let final_input_tokens = self
            .cache_result
            .input_tokens_after_cache(self.context_input_tokens);
...
impl BufferedStreamContext {
    /// 创建带 prompt cache usage 的缓冲流上下文
    pub fn new_with_cache(
        model: impl Into<String>,
        cache_result: CacheResult,
        thinking_enabled: bool,
    ...
    pub fn finish_and_get_all_events(&mut self) -> Vec<SseEvent> {
        ...
        let final_input_tokens = self
            .cache_result
            .input_tokens_after_cache(self.inner.context_input_tokens);
...
mod tests {
    ...
    #[test]
    fn test_stream_context_includes_cache_usage() {
        ...
        let mut ctx =
            StreamContext::new_with_cache("test-model", cache_result, false, HashMap::new());
        ...
        assert_eq!(start_usage["input_tokens"], 30);
        assert_eq!(start_usage["cache_creation_input_tokens"], 10);
        assert_eq!(start_usage["cache_read_input_tokens"], 20);
        ...
        let message_delta = final_events
            ...
            .unwrap();
        let delta_usage = &message_delta.data["usage"];
        assert_eq!(delta_usage["input_tokens"], 30);
        assert_eq!(delta_usage["cache_creation_input_tokens"], 10);
        assert_eq!(delta_usage["cache_read_input_tokens"], 20);
    }
